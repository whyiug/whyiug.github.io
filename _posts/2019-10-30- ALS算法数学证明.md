...
$$\operatorname{Loss}(X, Y)=\sum_{i, u}\left(r_{u i}-\boldsymbol{x}_{u} y_{i}^{T}\right)^{2}+\lambda\left(\sum_{u}\left\|\boldsymbol{x}_{u}\right\|^{2}+\sum_{i}\left\|\boldsymbol{y}_{i}\right\|^{2}\right)$$ ($$λ$$: 正规化系数)
...
**Preference**: **$$p_{u i}=\left\{\begin{array}{ll}{1} & {r_{u i}>0} \\ {0} & {r_{u i}=0}\end{array}\right.$$**

**Confidence**: $$c_{u i}=1+\alpha r_{u i}$$
...
计算步骤和`ALS`基本一样，这里只对$x_u$的算法做说明。
